...
import { applySlaCheck, getSlaState } from '../services/accountability.js';

const router = express.Router();

const VALID_STATUSES = new Set(['submitted', 'under_review', 'assigned', 'in_progress', 'resolved', 'reopened', 'escalated']);
const VALID_CATEGORIES = new Set(['roads', 'water', 'sanitation', 'streetlights', 'electricity', 'drainage', 'waste', 'public_health', 'other']);
...
function canAccess(user, grievance) { return user.role === 'official' ? user.panchayatId === grievance.panchayatId : grievance.citizenId === user.id; }
function getUser(req) { return store.users.get(req.userId); }
function enrich(grievance) { applySlaCheck(grievance); return { ...grievance, sla: getSlaState(grievance) }; }
...
router.post('/', authRequired, (req, res) => {
  ...
  if (!title || !description || !location || !panchayatId) return res.status(400).json({ error: 'title, description, location and panchayatId are required' });
  ...
  const grievance = { id: createId('grievance'), citizenId: user.id, panchayatId, title: title.trim(), description: description.trim(), location: location.trim(), evidence: evidence || [], category: aiTriage.category, severity: aiTriage.severity, status: 'under_review', assignedDepartment: aiTriage.department, assignedTo: null, createdAt: now.toISOString(), updatedAt: now.toISOString(), slaHours, slaDueAt: new Date(now.getTime() + slaHours * 60 * 60 * 1000).toISOString(), aiTriage, timeline: [] };
  ...
  res.status(201).json({ grievance: enrich(grievance) });
});

router.get('/', authRequired, (req, res) => {
  const user = getUser(req); const { status, severity, category } = req.query;
  const items = [...store.grievances.values()].filter((item) => canAccess(user, item)).filter((item) => !status || item.status === status).filter((item) => !severity || item.severity === severity).filter((item) => !category || item.category === category).map(enrich).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  res.json({ grievances: items });
});

router.get('/:id', authRequired, (req, res) => {
  const user = getUser(req); const grievance = store.grievances.get(req.params.id);
  if (!grievance) return res.status(404).json({ error: 'Grievance not found' });
  if (!canAccess(user, grievance)) return res.status(403).json({ error: 'Access denied' });
  res.json({ grievance: enrich(grievance) });
});

router.patch('/:id/assign', authRequired, (req, res) => {
  const user = getUser(req); const grievance = store.grievances.get(req.params.id);
  if (!grievance) return res.status(404).json({ error: 'Grievance not found' });
  if (!user || user.role !== 'official' || user.panchayatId !== grievance.panchayatId) return res.status(403).json({ error: 'Only the responsible Panchayat official can assign grievances' });
  ...
  if (['under_review', 'submitted'].includes(grievance.status)) grievance.status = 'assigned';
...
router.patch('/:id/status', authRequired, (req, res) => {
  const user = getUser(req); const grievance = store.grievances.get(req.params.id);
  if (!grievance) return res.status(404).json({ error: 'Grievance not found' });
  if (!user || user.role !== 'official' || user.panchayatId !== grievance.panchayatId) return res.status(403).json({ error: 'Only the responsible Panchayat official can update status' });
  const { status, note } = req.body;
  if (!VALID_STATUSES.has(status)) return res.status(400).json({ error: 'Invalid grievance status' });
  grievance.status = status; grievance.updatedAt = new Date().toISOString();
...
router.patch('/:id/triage', authRequired, (req, res) => {
  ...
  if (category) grievance.category = category; if (severity) grievance.severity = severity; if (department) grievance.assignedDepartment = department;
  ...
  grievance.updatedAt = new Date().toISOString(); addTimelineEvent(grievance, 'triage_override', reason || 'AI triage manually adjusted by official', user.id);
  res.json({ grievance: enrich(grievance) });
});
